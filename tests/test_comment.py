...
def test_comment_collection_lin21():
	...
	ldf = ldfparser.parseLDF(path, captureComments=True)
	assert len(ldf.comments) >= 0
	assert "// Source: https://lin-cia.org/fileadmin/microsites/lin-cia.org/resources/documents/LIN-Spec_Pac2_1.pdf' in ldf.comments" in ldf.comments
...
def test_comment_collection_lin22():
	...
	ldf = ldfparser.parseLDF(path, captureComments=True)
	assert len(ldf.comments) >= 0
	assert "// Source: https://lin-cia.org/fileadmin/microsites/lin-cia.org/resources/documents/LIN_2.2A.pdf' in ldf.comments" in ldf.comments
